...
import { X, RefreshCw, Puzzle, FolderOpen, Trash2, ToggleLeft, ToggleRight, TerminalSquare } from '@lucide/vue';
    ...
    >
      <div class="settings-panel relative backdrop-blur-2xl w-full max-w-3xl">
          ...
          <div class="flex items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2">
              ...
              <span
                ...
                :class="p.syncStatus.value.deviceConnected ? 'bg-green-500/15 text-green-400' : 'bg-surface-variant/40 text-on-surface-variant'"
              >
                {{ p.syncStatus.value.deviceConnected ? $t('plugins.sync.connected') : $t('plugins.sync.disconnected') }}
              </span>
              <button
                @click="p.refresh"
              ...
              >
                <RefreshCw class="w-4 h-4 text-on-surface-variant" :class="{ 'animate-spin': p.loading.value }" />
              </button>
            </div>
          </div>
        </div>
        ...
        <div class="settings-scrollbar p-6 overflow-y-auto overscroll-contain max-h-[65vh]">
          <p v-if="p.error.value" class="mb-4 px-4 py-2 rounded-lg bg-red-500/10 text-red-400 text-sm">
            {{ p.error.value }}
          </p>

          <div v-if="p.loading.value && p.plugins.value.length === 0" class="py-16 text-center text-on-surface-variant text-sm">
            {{ $t('plugins.loading') }}
          </div>
          ...
          <div v-else class="space-y-4">
            <!-- Install hint -->
            <div class="flex items-center justify-between px-4 py-2 rounded-lg bg-surface-variant/20 text-xs text-on-surface-variant">
            ...
            >
              <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                  <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <span
                      ...
                      :class="plugin.runtime === 'wasm' ? 'bg-purple-500/15 text-purple-400' : 'bg-amber-500/15 text-amber-400'"
                    >
                    ...
                    <span v-if="plugin.dspNode" class="px-2 py-0.5 rounded-md text-[10px] bg-green-500/15 text-green-400">
                      {{ $t('plugins.inChain') }}
                    </span>
                  </div>
                  <p class="text-xs text-on-surface-variant mt-1 font-mono">{{ plugin.id }} · v{{ plugin.version }}</p>
                  <p v-if="plugin.description" class="text-xs text-on-surface-variant/80 mt-1 line-clamp-2">
                    {{ plugin.description }}
                  </p>
                ...
                <div class="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    ...
                    :class="plugin.enabled ? 'bg-primary/20 text-primary' : 'bg-surface-variant/40 text-on-surface-variant'"
                  >
              ...
              <div v-if="showLogsFor === plugin.id" class="mt-3 pt-3 border-t border-surface-variant/20">
                ...
                <pre class="mt-2 max-h-40 overflow-y-auto bg-black/30 rounded-lg p-3 text-[11px] font-mono text-green-300/90 whitespace-pre-wrap">{{ logLines.join('\n') || $t('plugins.noLogs') }}</pre>
              </div>
            </div>
          </div>
        </div>
